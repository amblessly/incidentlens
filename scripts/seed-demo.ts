...
async function main() {
  ...
  if (existing.length > 0) {
    ...
    for (const inc of existing) {
      await db().prepare("DELETE FROM investigation_runs WHERE incident_id = ?").run(inc.id);
      await db().prepare("DELETE FROM investigation_steps WHERE run_id IN (SELECT id FROM investigation_runs WHERE incident_id = ?)").run(inc.id);
      await db().prepare("DELETE FROM evidence_relationships WHERE evidence_id IN (SELECT id FROM evidence WHERE incident_id = ?)").run(inc.id);
      await db().prepare("DELETE FROM evidence WHERE incident_id = ?").run(inc.id);
      await db().prepare("DELETE FROM hypotheses WHERE incident_id = ?").run(inc.id);
      await db().prepare("DELETE FROM incident_events WHERE incident_id = ?").run(inc.id);
      await db().prepare("DELETE FROM incidents WHERE id = ?").run(inc.id);
    }
  }
