...
export async function createApiKey(
  ...
  return {
    row: (await db().prepare("SELECT * FROM api_keys WHERE id = ?").get(id)) as ApiKeyRow,
    secret: key,
  };
}
...
export async function verifyApiKey(key: string): Promise<ApiKeyRow | null> {
  ...
  const row = (await db().prepare("SELECT * FROM api_keys WHERE key_hash = ?").get(hash)) as
    | ApiKeyRow
    | undefined;
  if (!row) return null;
  if (row.revoked_at) return null;
  if (row.expires_at && Date.now() > new Date(row.expires_at).getTime()) return null;
  await db().prepare("UPDATE api_keys SET last_used_at = ? WHERE id = ?").run(nowIso(), row.id);
  return (await db().prepare("SELECT * FROM api_keys WHERE id = ?").get(row.id)) as ApiKeyRow;
}

export async function revokeApiKey(id: string): Promise<void> {
  await db().prepare("UPDATE api_keys SET revoked_at = ? WHERE id = ?").run(nowIso(), id);
}

/** Revoke the old key and issue a new one. */
export async function rotateApiKey(id: string): Promise<CreatedApiKey | null> {
  const row = (await db().prepare("SELECT * FROM api_keys WHERE id = ?").get(id)) as
    | ApiKeyRow
    | undefined;
  if (!row) return null;
